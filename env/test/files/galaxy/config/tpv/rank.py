...
if len(candidate_destinations) > 1:
    ...
    log.debug(f"#### queued_counts_by_destination: {queued_counts_by_destination}")
    log.debug(f"#### candidate_destinations: {candidate_destinations}")
    for d in candidate_destinations:
        log.debug(f"#### {d.id} {-1 * d.score(entity)}, {1 * queued_counts_by_destination.get(d.id, 0)}, {random.randint(0,9)}")
    candidate_destinations.sort(key=lambda d: (-1 * d.score(entity), 1 * queued_counts_by_destination.get(d.id, 0), random.randint(0,9)))
    #raise JobNotReadyException()
    final_destinations = candidate_destinations
else:
    final_destinations = candidate_destinations
log.debug(f"#### final_destinations: {final_destinations}")
final_destinations
