...
VALID_DESTINATIONS = STAMPEDE_DESTINATIONS
RESOURCE_KEYS = ('stampede_compute_resource',)
FAILURE_MESSAGE = 'This tool could not be run because of a misconfiguration in the Galaxy job running system, please report this error'

def stampede_select( app, tool, job, user_email ):
    destination_id = STAMPEDE_DESTINATION
    tool_id = tool.id
    if '/' in tool.id:
    ...
    log.debug("(%s) stampede_select dynamic plugin returning '%s' destination", job.id, destination_id)
